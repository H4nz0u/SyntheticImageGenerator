...
from .utilities import create_transformation, create_positionDeterminer, create_filter, create_annotation, logger, get_cached_dataframe, create_blending
...
class Config:
    ...
    def merge_configs(self):
        ...
        self.config = {
            ...
            'blending': self._parse_blending(self.transform_config.get('blending', 'StandardBlending')),
    ...
    def _parse_blending(self, blending: str) -> BaseBlending:
        return create_blending(blending)
    ...
    def _load_dataframes(self):
        """
        Loads and caches all DataFrames as per the configuration.
        """
        dataframes_config = self.data_config.get("dataframes", {})
        
        for name, config in dataframes_config.items():
            path = config.get("path")
            correlated_columns = config.get("correlated_columns", [])
            
            if not path:
                logger.warning(f"No path specified for DataFrame '{name}'. Skipping.")
                continue
            
            try:
                _ = get_cached_dataframe(name, path, correlated_columns)
                logger.info(f"DataFrame '{name}' loaded and cached successfully.")
            except Exception as e:
                logger.error(f"Failed to load DataFrame '{name}' from '{path}': {e}")
                raise    
...
class OptimizationConfig(Config):
    ...
    def __init__(self, seed: int = 42):
        self.config = {
            'transformations': {},
            'filters': [],
            'blending': self._parse_blending("FeatheredBlending"),
            'positioning': None,
            'foreground_objects': {
    ...
    def set_initial_parameters(self, params: Dict[str, Any]) -> None:
        self.optimization_dict = params
        self.serialize()

    def serialize(self):
        for key, obj in self.optimization_dict.items():  
            """
            Convert numpy types to their native Python equivalents for JSON serialization.
            """
            if isinstance(obj, (np.integer)):
                self.optimization_dict[key] = int(obj)
            elif isinstance(obj, (np.floating)):
                self.optimization_dict[key] = float(obj)
            elif isinstance(obj, (np.ndarray,)): # convert np.array to list
                self.optimization_dict[key] = obj.tolist()
