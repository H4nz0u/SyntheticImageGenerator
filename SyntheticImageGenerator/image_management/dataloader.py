...
import cv2
from SyntheticImageGenerator.utilities import logger

class DataLoader:
    def __init__(self, root_path: str, seed: int = 42) -> None:
        random.seed(seed)
...
class ImageDataLoader(DataLoader):
    def __init__(self, root_path: str, objects: Dict[str, str], seed: int = 42) -> None:
        super().__init__(root_path, seed)
        self.objects: Dict[str, str] = {obj: os.path.join(root_path, path) for obj, path in objects.items()}
        self.object_image_paths = self._get_image_paths()

    def _get_image_paths(self) -> Dict[str, List[str]]:
    ...
    def get_image(self, image_type: str|None) -> ImgObject:
...
class BackgroundDataLoader(DataLoader):
    ...
    @lru_cache(maxsize=1024)
    def _serve_background(self, image_path: str) -> cv2.typing.MatLike:
